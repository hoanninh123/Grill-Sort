import { GameState, PlayerInfo, Room } from "./types";
...
export class RoomService {
  ...
  createRoom(owner: PlayerInfo): Room {
  ...
  joinRoom(roomId: string, player: PlayerInfo): Room | null {
  ...
  applyMove(roomId: string, from: number, to: number): GameState {
    const room = this.rooms.get(roomId);
    if (!room) {
  ...
  private validateMove(grills: string[][], from: number, to: number): void {
    if (from === to) {
  ...
  private createInitialState(): GameState {
    ...
    const grills: string[][] = Array.from({ length: DEFAULT_GRILL_COUNT }, () => []);
  ...
  private computeIsCompleted(grills: string[][]): boolean {
